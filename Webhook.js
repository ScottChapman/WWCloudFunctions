		...
		if (body.hasOwnProperty("type") && expansion.hasOwnProperty(body.type)) {
			...
			ow.actions.invoke({
				...
				if (body.type.startsWith("message-annotation"))
					body.ownEvent = resp.response.result.data.message.createdBy.id === params.AppInfo.AppId;
...
function cleanUpEvent(event, params) {
	...
	if (_.get(event,"annotationPayload.targetAppId"))
		event.ownEvent = event.annotationPayload.targetAppId === params.AppInfo.AppId;
	else
		event.ownEvent = event.userId === params.AppInfo.AppId;
	...
	return new Promise((resolve, reject) => {
		...
		if (_.get(req.body, "type") === "verification") {
			...
			if (_.get(req.body, "annotationType") === "actionSelected" && req.body.ownEvent) {
				var triggerName = params.WatsonWorkspace.ActionSelected;
				if (req.body.annotationPayload.actionId.startsWith(params.WatsonWorkspace.ButtonSelectedPrefix)) {
					triggerName = params.WatsonWorkspace.ButtonSelected;
					var action = req.body.annotationPayload.actionId.substr(params.WatsonWorkspace.ButtonSelectedPrefix.length)
					try {
				...
				expandEvent(req.body, params.WatsonWorkspace, ow).then(body => {
					...
					// Send event to topic
					ow.triggers.invoke({
						name: body.ownEvent ? params.WatsonWorkspace.OwnEventTrigger : params.WatsonWorkspace.OthersEventTrigger,
						params: body
					}).then(result => {
						resolve({
							statusCode: 200,
